...
{ gesetz, abkuerzung, paragraph, absatz,
  source_url, source_repo, commit_sha, retrieved_at, licence_note,
  stand, title, is_synthetic }
```
...
- **Pin to a revision.** `commit_sha` records the exact upstream commit the text
  came from. Without it, "we ingested the AO" is unfalsifiable.
- **`stand` is the law's own build date, not our fetch date.** A document fetched
  today can carry law from eighteen months ago. The UI shows `stand`, because
  cited-but-stale is a real failure mode in tax.
- `corpus/PROVENANCE.md` is GENERATED by `pnpm corpus:build`. Never hand-edit it —
  a hand-maintained provenance table is one that eventually lies.
